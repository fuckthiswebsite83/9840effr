...
local function createESPForModel(model, drawings, processedModels, connections, espSize, espColor, renderDistance)
	...
	local primaryPart = model:WaitForChild("PrimaryPart", 5) or model:FindFirstChildWhichIsA("BasePart");
...
local function clearProcessedModels(processedModels)
	for k in pairs(processedModels) do
		processedModels[k] = nil;
	end
end
local function clearAll(drawings, processedModels, connections)
	clearDrawings(drawings);
	clearConnections(connections);
	clearProcessedModels(processedModels);
end
local function manageItemESP()
	clearAll(activeItemDrawings, processedItemModels, itemConnections);
...
local function manageVehicleESP()
	clearAll(activeVehicleDrawings, processedVehicleModels, vehicleConnections);
	...
	vehicleConnection = workspace.Vehicles.Spawned.ChildAdded:Connect(function(child)
	...
	table.insert(vehicleConnections, vehicleConnection);
end
...
local function createPlayerESPElements(model, espSize, espColor)
	...
	if primaryPart then
		...
		local stats = player and player:WaitForChild("Stats", 5);
		...
		return {Model=model,PrimaryPart=primaryPart,CombinedLabel=combinedLabel,Box=box};
	end
end
local function highlightp(target)
	if not HighlightESPEnabled then
		return;
	end
...
local function managePlayerESP()
	clearAll(activePlayerDrawings, processedPlayerModels, playerConnections);
	...
	local function addPlayerESP(player)
		if (player ~= LocalPlayer) then
			...
			table.insert(playerConnections, charAddedConnection);
			if player.Character then
...
local function createZombieESPForModel(model, drawings, processedModels, connections, espSize, espColor, renderDistance)
	...
	if primaryPart then
		...
		connection = RunService.RenderStepped:Connect(function()
			...
			local localCharacter = Players.LocalPlayer.Character;
			local humanoidRootPart = localCharacter and localCharacter:WaitForChild("HumanoidRootPart", 5);
...
local function manageZombieESP()
	clearAll(activeZombieDrawings, processedZombieModels, zombieConnections);
	...
	zombieConnection = workspace.Zombies.Mobs.ChildAdded:Connect(function(child)
		if child:IsA("Model") then
	...
	table.insert(zombieConnections, zombieConnection);
end
...
LocalPlayer.CharacterRemoving:Connect(function()
	...
	clearAll(activePlayerDrawings, processedPlayerModels, playerConnections);
	clearAll(activeZombieDrawings, processedZombieModels, zombieConnections);
	clearAll(activeEventDrawings, processedEventModels, eventConnections);
end);
...
setreadonly(mt, true);
ItemESPGroupBox:AddToggle("ItemESP", {Text="Item ESP",Default=false,Tooltip="Toggle item ESP on or off",Callback=function(Value)
...
ItemESPGroupBox:AddLabel("ESP Color"):AddColorPicker("ItemESPColor", {Default=Color3.new(1, 1, 1),Title="Select ESP color",Callback=function(Value)
	ItemESPColor = Value;
	for _, drawing in ipairs(activeItemDrawings) do
		drawing.Color = ItemESPColor;
	end
end});
ItemESPGroupBox:AddSlider("ItemESPSize", {Text="ESP Size",Default=20,Min=10,Max=50,Rounding=1,Compact=false,Callback=function(Value)
	ItemESPSize = Value;
	for _, drawing in ipairs(activeItemDrawings) do
		drawing.Size = ItemESPSize;
	end
end});
Options.ItemESPSize:OnChanged(function()
end);
...
VehicleESPGroupBox:AddToggle("VehicleESP", {Text="Vehicle ESP",Default=false,Tooltip="Toggle vehicle ESP on or off",Callback=function(Value)
...
VehicleESPGroupBox:AddLabel("ESP Color"):AddColorPicker("VehicleESPColor", {Default=Color3.new(1, 1, 1),Title="Select ESP color",Callback=function(Value)
	VehicleESPColor = Value;
	for _, drawing in ipairs(activeVehicleDrawings) do
		drawing.Color = VehicleESPColor;
	end
end});
VehicleESPGroupBox:AddSlider("VehicleESPSize", {Text="ESP Size",Default=20,Min=10,Max=50,Rounding=1,Compact=false,Callback=function(Value)
	VehicleESPSize = Value;
	for _, drawing in ipairs(activeVehicleDrawings) do
		drawing.Size = VehicleESPSize;
	end
end});
VehicleESPGroupBox:AddSlider("VehicleRenderDistance", {Text="Render Distance",Default=1000,Min=100,Max=5000,Rounding=1,Compact=false,Callback=function(Value)
...
Options.VehicleRenderDistance:OnChanged(function()
end);
...
PlayerESPGroupBox:AddToggle("PlayerESP", {Text="Player ESP",Default=false,Tooltip="Toggle player ESP on or off",Callback=function(Value)
	PlayerESPEnabled = Value;
	managePlayerESP();
end});
PlayerESPGroupBox:AddToggle("HighlightESP", {Text="Chams",Default=false,Tooltip="Toggle chams on or off",Callback=function(Value)
...
PlayerESPGroupBox:AddToggle("ChamsWallcheck", {Text="Chams Wallcheck",Default=false,Tooltip="Toggle Chams Wallcheck on or off",Callback=function(Value)
...
PlayerESPGroupBox:AddToggle("PlayerESPText", {Text="Text ESP",Default=false,Tooltip="Toggle text ESP on or off",Callback=function(Value)
...
PlayerESPGroupBox:AddToggle("PlayerESPBox", {Text="Box ESP",Default=false,Tooltip="Toggle box ESP on or off",Callback=function(Value)
...
PlayerESPGroupBox:AddLabel("ESP Color"):AddColorPicker("PlayerESPColor", {Default=Color3.new(1, 1, 1),Title="Select ESP color",Callback=function(Value)
...
PlayerESPGroupBox:AddSlider("PlayerESPSize", {Text="ESP Size",Default=20,Min=10,Max=50,Rounding=1,Compact=false,Callback=function(Value)
	PlayerESPSize = Value;
	for _, drawing in ipairs(activePlayerDrawings) do
		drawing.CombinedLabel.Size = PlayerESPSize;
	end
end});
PlayerESPGroupBox:AddSlider("PlayerRenderDistance", {Text="Render Distance",Default=1000,Min=100,Max=5000,Rounding=1,Compact=false,Callback=function(Value)
	PlayerRenderDistance = Value;
end});
WorldMiscGroupBox:AddButton({Text="Fullbright",Func=function()
	configureLighting();
end});
WorldMiscGroupBox:AddButton({Text="No Foilage",Func=function()
...
ZombieESPGroupBox:AddToggle("ZombieESP", {Text="Zombie ESP",Default=false,Tooltip="Toggle zombie ESP on or off",Callback=function(Value)
	ZombieESPEnabled = Value;
	manageZombieESP();
end});
ZombieESPGroupBox:AddLabel("ESP Color"):AddColorPicker("ZombieESPColor", {Default=Color3.new(1, 1, 1),Title="Select ESP color",Callback=function(Value)
	ZombieESPColor = Value;
	for _, drawing in ipairs(activeZombieDrawings) do
		drawing.Color = ZombieESPColor;
	end
end});
ZombieESPGroupBox:AddSlider("ZombieESPSize", {Text="ESP Size",Default=20,Min=10,Max=50,Rounding=1,Compact=false,Callback=function(Value)
	ZombieESPSize = Value;
	for _, drawing in ipairs(activeZombieDrawings) do
		drawing.Size = ZombieESPSize;
	end
end});
ZombieESPGroupBox:AddSlider("ZombieRenderDistance", {Text="Render Distance",Default=1000,Min=100,Max=5000,Rounding=1,Compact=false,Callback=function(Value)
	ZombieRenderDistance = Value;
	if ZombieESPEnabled then
		manageZombieESP();
	end
end});
EventESPGroupBox:AddToggle("EventESP", {Text="Event ESP",Default=false,Tooltip="Toggle event ESP on or off",Callback=function(Value)
	EventESPEnabled = Value;
	handleEventESP();
end});
EventESPGroupBox:AddLabel("ESP Color"):AddColorPicker("EventESPColor", {Default=Color3.new(1, 1, 1),Title="Select ESP color",Callback=function(Value)
	EventESPColor = Value;
	for _, drawing in ipairs(activeEventDrawings) do
		drawing.Color = EventESPColor;
	end
end});
EventESPGroupBox:AddSlider("EventESPSize", {Text="ESP Size",Default=20,Min=10,Max=50,Rounding=1,Compact=false,Callback=function(Value)
	EventESPSize = Value;
	for _, drawing in ipairs(activeEventDrawings) do
		drawing.Size = EventESPSize;
	end
end});
EventESPGroupBox:AddSlider("EventRenderDistance", {Text="Render Distance",Default=1000,Min=100,Max=5000,Rounding=1,Compact=false,Callback=function(Value)
	EventRenderDistance = Value;
	if EventESPEnabled then
		handleEventESP();
	end
end});
GunModsGroupBox:AddButton({Text="No Spread No Recoil V.1",Func=function()
	...
	repeat
		task.wait();
	until ReplicatedFirst:FindFirstChild("Framework") 
	local Framework = require(ReplicatedFirst.Framework);
	repeat
		task.wait();
	until Framework.Classes.Players.get() 
...
GunModsGroupBox:AddButton({Text="No Spread No Recoil V.2",Func=function()
	local ReplicatedFirst = game:GetService("ReplicatedFirst");
	local Framework = require(ReplicatedFirst.Framework);
	local Bullets = Framework.Libraries.Bullets;
	local GetSpreadAngle = getupvalue(Bullets.Fire, 1);
...
MovementGroupBox:AddToggle("JumpHack", {Text="Jump Hack",Default=false,Tooltip="Toggle jump hack on or off",Callback=function(Value)
...
MovementGroupBox:AddSlider("JumpHeight", {Text="Jump Height",Default=50,Min=10,Max=200,Rounding=1,Compact=false,Callback=function(Value)
	JumpHeight = Value;
end});
...
MovementGroupBox:AddToggle("TpWalking", {Text="Teleport Walking",Default=false,Tooltip="Toggle teleport walking on or off",Callback=function(Value)
	TpWalkingEnabled = Value;
	if TpWalkingEnabled then
		startTPWalking();
	end
end});
MovementGroupBox:AddSlider("TpWalkSpeed", {Text="Teleport Walk Speed",Default=10,Min=1,Max=50,Rounding=1,Compact=false,Callback=function(Value)
	TpWalkSpeed = Value;
end});
ExploitsGroupBox:AddButton({Text="Anti-Zombie",Func=function()
...
HitboxExpanderGroupBox:AddToggle("HitboxExpander", {Text="Enable Hitbox Expander",Default=false,Tooltip="Toggle hitbox expander on or off",Callback=function(Value)
...
HitboxExpanderGroupBox:AddSlider("HitboxSize", {Text="Hitbox Size",Default=4,Min=1,Max=10,Rounding=1,Compact=false,Callback=function(Value)
	HitboxSize = Value;
	if HitboxExpanderEnabled then
		for _, player in pairs(Players:GetPlayers()) do
			if ((player ~= LocalPlayer) and player.Character) then
...
local WatermarkConnection = game:GetService("RunService").RenderStepped:Connect(function()
	...
	Library:SetWatermark(("warp.space | %s fps | %s ms"):format(math.floor(FPS), math.floor(game:GetService("Stats").Network.ServerStatsItem["Data Ping"]:GetValue())));
end);
